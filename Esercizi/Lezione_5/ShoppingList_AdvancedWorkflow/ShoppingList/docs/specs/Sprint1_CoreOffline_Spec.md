...
| **Data** | 2026-04-14 |
| **Stato** | 🔄 **In esecuzione — Fasi plan 0-3 completate, 4-9 da fare** (update 2026-04-14 pomeriggio) |
| **Sprint target** | Sprint 1 — `docs/piano-sviluppo.md` |
| **Metodologia** | Spec-Driven Development con `superpowers:brainstorming` + `superpowers:subagent-driven-development` |
...
| **Plan implementativo** | [`docs/plans/Sprint1_CoreOffline_Plan.md`](../plans/Sprint1_CoreOffline_Plan.md) — vedi "Progress Summary" in testa al plan per stato per-task |

---

## ⚠️ Stato di esecuzione (update 2026-04-14)

La spec sotto è **approvata e immutata**. L'esecuzione è stata avviata usando
`superpowers:subagent-driven-development` seguendo il plan. Stato:

**Completato (Fasi 0-3 del plan):**
- Phase 0 — Setup dipendenze (`@radix-ui/react-dialog@^1`)
- Phase 1 — Utilities di fondazione: `id-utils`, `validation` (+**`validateItemPatch`** aggiunto dalla code review), `diff` (con fix union keys), `_internal/domain-error`, `_internal/map-db-error`
- Phase 2 — Repositories: `list-repository`, `item-repository`, `change-log-repository` con smoke test 5+5
- Phase 3 — Services: `listService` (16 test) + `itemService` (21 test). Copertura 100%.

**Stato quality gates:** 64 test verdi | typecheck ✅ | lint ✅

**Deviazioni da spec emerse in esecuzione:**
1. `validation.ts` espone un ulteriore helper `validateItemPatch` per patch parziali senza nome — non nella spec originale ma necessario per chiudere un gap di validazione in `updateItem` (cattura quantity/notes invalidi senza name). La firma `validateItemInput` resta invariata.
2. `item-repository.ts` usa un piccolo helper privato `itemsTable(tx)` (idem per `list-repository` e `change-log-repository`) per normalizzare `tx?.table<T>('x')` vs `db.items`. Deviazione di typing interna, nessun impatto sul contratto pubblico.
3. `item-service.ts` è circa a 290 LOC, sopra il target di 200 ma sotto i 400 max. Non refactorizzato per non divergere dalla spec; potenziale extract di `buildItemLogEntry(...)` se in Phase 9 si vuole rientrare nel target.

**Da fare (Fasi 4-9 del plan):**
- Phase 4 — Hooks (`use-lists`, `use-items`, `use-deleted-items`) con tests
- Phase 5 — UI common (9 componenti + rewrite `ui-store`)
- Phase 6 — UI liste (ListCard, ListForm, ArchivedSection, rewrite HomePage)
- Phase 7 — UI articoli (ItemRow/Form/QuickAddBar/TrashRow + ListPage + TrashPage)
- Phase 8 — Wire & Verify (routing in `app.tsx`, smoke test offline, update `mappa-progetto.md`)
- Phase 9 — Final verification quality gates

**Riprendere da:** Phase 4 — Task 4.1 `use-lists.ts con TDD` nel plan.
