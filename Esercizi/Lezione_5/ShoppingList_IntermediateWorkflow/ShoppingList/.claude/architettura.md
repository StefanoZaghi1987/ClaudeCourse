...
> ✅ **CANONICAL — Aggiornamento brainstorming 2026-04-13**
> Questo file è la **fonte autoritativa** per la struttura cartelle `src/` e per i pattern architetturali (layer, store, routing, optimistic update). La struttura layer-sliced presentata in [`development_plan.md` §2.2](./development_plan.md#22-struttura-directory) è stata **deprecata** in favore della struttura **feature-sliced** descritta qui in §"Struttura Cartelle Dettagliata".
>
> Quando si esegue un task degli sprint S1-S5 che cita un path nel vecchio layout (es. `src/services/listService.ts`), tradurlo all'equivalente feature-sliced usando la **tabella di traduzione** in `development_plan.md` §2.2.
