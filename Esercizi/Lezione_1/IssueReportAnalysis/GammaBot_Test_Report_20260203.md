...
```
Framing 'http://10.0.4.49:5000/' violates the following Content Security Policy directive:
...
Il problema è doppio: (1) viene utilizzato l'indirizzo IP del server (`10.0.4.49`) invece del domain name, e (2) la direttiva CSP `default-src 'self'` non include la direttiva `frame-src` necessaria per consentire l'embedding di iframe da origini autorizzate.
...
2. Sostituire il riferimento a `http://10.0.4.49:5000/` con il corrispondente domain name (allineato al fix già segnalato nei bug #2 e #3 relativi all'uso dell'IP nel reset password).
3. Valutare se il servizio di preview PDF (`10.0.4.49:5000`) debba essere accessibile tramite proxy del backend principale, eliminando così la necessità di gestire origini cross-domain nel frontend.
...
```
GET http://git-vtf-gb01:3000/api/bestpractices?  →  403 Forbidden
{"error":"Permesso negato","required":"bestpractices.read"}

GET http://git-vtf-gb01:3000/api/conversations/stats?days=30  →  403 Forbidden
...
- **Nomenclatura URL con IP invece di domain name** (Issue #2, #3): sistematicamente, i link presenti nelle email di sistema (reset password, OTP) usano l'indirizzo IP del server (`http://10.0.4.47:3000/`) invece del domain name. Da risolvere a livello di configurazione del servizio di notifica.
