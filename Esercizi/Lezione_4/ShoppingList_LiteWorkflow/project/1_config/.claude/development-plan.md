...
| Fase | Nome | Task | Descrizione |
|------|------|------|-------------|
| **1** | **Fondamenta** | 1.1 - 1.4 | Setup progetto, modelli dati, database layer, event system |
| **2** | **Core Offline** | 2.1 - 2.5 | Services, CRUD liste, CRUD articoli, autocomplete, database articoli |
| **3** | **UI/UX** | 3.1 - 3.6 | Component system, HomeView, ListView, modal/form, routing, stili |
| **4** | **Auth & Sharing** | 4.1 - 4.3 | Guest mode, registrazione, condivisione liste |
| **5** | **Sync & PWA** | 5.1 - 5.4 | Sync engine, conflict resolution, Service Worker, PWA manifest |
| **6** | **Polish & Test** | 6.1 - 6.3 | Testing, performance, bug fix e rifinitura |
...
**Obiettivo**: Inizializzare il progetto Vite con TypeScript, Tailwind CSS e tutte le dipendenze.

**Documentazione da leggere**: `architecture.md` (sezioni "Setup Iniziale", "Struttura Progetto")

**Azioni richieste**:

1. **Inizializzare progetto Vite** nella directory `src/`:
   ```bash
...
7. **Configurare file base**:
   ...
   - `src/main.ts` (entry point vuoto con import CSS)
   - `.gitignore` (node_modules, dist, .env)
   - `.env.example`

8. **Configurare linting** (opzionale ma consigliato):
   - ESLint + TypeScript ESLint
   - Prettier

**Criteri di accettazione**:
- [ ] `pnpm dev` avvia il dev server senza errori
- [ ] `pnpm build` compila senza errori
- [ ] TypeScript strict mode attivo e funzionante
- [ ] Tailwind CSS funzionante (testare con una classe su index.html)
- [ ] Path aliases funzionanti (testare con un import)
- [ ] Struttura directory creata

**Output atteso**: Progetto funzionante con dev server, build, e tutte le configurazioni.
...
3. **Creare repository per ogni entita** (pattern Repository):
...
4. **Creare `src/db/index.ts`** - Re-export del singleton db e di tutti i repository
...
- [ ] Seed function popola articoli solo al primo avvio
- [ ] Tutti i repository implementano le operazioni CRUD
...
**Output atteso**: Tutti i file in `src/db/` funzionanti e testabili.
...
1. **Creare `src/utils/events.ts`** - Event Bus:
   ```typescript
   // Definire costanti eventi:
   // AppEvents = {
   //   LIST_CREATED, LIST_UPDATED, LIST_DELETED,
   //   ITEM_ADDED, ITEM_UPDATED, ITEM_CHECKED, ITEM_DELETED,
   //   ARTICLE_CREATED,
   //   SYNC_STATUS_CHANGED, SYNC_COMPLETED, SYNC_ERROR,
   //   AUTH_STATE_CHANGED,
...
2. **Creare `src/utils/dom.ts`** - DOM helpers:
   - `createElement<K>(tag, attributes?, children?): HTMLElement`
   - `querySelector<T>(selector, parent?): T | null`
   - `sanitizeHTML(input: string): string` (usa textContent per escape)
   - `show(element): void`, `hide(element): void`, `toggle(element): void`
...
3. **Creare `src/utils/validators.ts`**:
   ...
   - `sanitizeInput(input: string): string` (trim + rimuovi tag HTML)
...
6. **Creare `src/utils/debounce.ts`**:
   - `debounce<T>(func: T, wait: number): T` - per search e sync

7. **Aggiornare `src/utils/uuid.ts`** (se non gia creato nel Task 1.3):
   - `generateUUID(): string` - usando `crypto.randomUUID()`
   - `generateSecureToken(): string` - per invite tokens (32 chars hex)

**Criteri di accettazione**:
- [ ] EventBus funziona con emit/on/off/once
- [ ] Tutti gli helper sono tipizzati correttamente (no `any`)
- [ ] Debounce ritarda correttamente l'esecuzione
- [ ] Validators validano correttamente email, password, nomi
- [ ] LocalStorage wrapper gestisce serializzazione/deserializzazione JSON

**Output atteso**: Tutti i file in `src/utils/` funzionanti.
